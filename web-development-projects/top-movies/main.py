...
class RateMovieForm(FlaskForm):
    rating = StringField(
        label="Your rating out of 10 eg. 7.5", validators=[DataRequired()]
    )
...
def home():
    all_movies = (
        db.session.execute(db.select(Movie).order_by(Movie.rating)).scalars().all()
    )
...
def add_movie():
    ...
    if form.validate_on_submit():
        ...
        params = {"apikey": OMDB_API_KEY, "s": movie_title, "r": "json"}
