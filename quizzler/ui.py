...
class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.score_label = Label(
            text="Score=0", fg="white", font=("Monolisa", 12, "normal"), bg=THEME_COLOR
        )
        ...
        self.right_btn = Button(
            image=right_img, highlightthickness=0, command=self.right_pressed
        )
        self.right_btn.grid(row=2, column=0)

        wrong_img = PhotoImage(file="images/false.png")
        self.wrong_btn = Button(
            image=wrong_img, highlightthickness=0, command=self.wrong_pressed
        )
